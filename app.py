...
@st.cache_resource
def load_artifacts():
    model = joblib.load("results/gb_model.pkl")
    X_test = pd.read_csv("results/X_test_sample.csv", index_col=0)
    y_test = pd.read_csv("results/y_test_sample.csv", index_col=0).squeeze("columns")
    y_pred_proba = np.load("results/y_pred_proba.npy")
    threshold_df = pd.read_csv("results/threshold_matrix.csv")
    dashboard_df = pd.read_csv("results/dashboard_diagnosis_summary.csv")
    with open("results/chosen_threshold.txt") as f:
